...
app = typer.Typer(help="🛡️ QuantumGuard: Post-Quantum Cryptography Auditor")
console = Console()
...
def scan(
    ...
    console.print(
        Panel(
            f"[bold blue]🛡️ QuantumGuard PQC Scanner[/bold blue]\n"
    ...
    if result.get("status") == "error":
        console.print(f"[bold red]❌ Connection Error:[/bold red] {result.get('message')}")
        raise typer.Exit(1)
    ...
    # 7. Final Verdict
    if grade in ["A+", "A"]:
        console.print(f"[bold green]✅ PASS:[/bold green] {hostname} is prepared for Y2Q (Year to Quantum).")
    else:
        console.print(f"[bold yellow]⚠️ WARNING:[/bold yellow] {hostname} is vulnerable to Store-Now-Decrypt-Later attacks.")
